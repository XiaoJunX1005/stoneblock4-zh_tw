...
- Leave：離開
- Disband：解散
- Transfer ownership：移交隊長職位
- Permission：權限
- Public：公開
- Private：私人
- Ally：盟友
- Enemy：敵對
- Chat redirected：聊天訊息導向（隊伍頻道/一般頻道）
- Stage：階段
- Property：屬性

## XNet（你前面已定的方向，放這裡方便一致）
- Network：網路
- Controller：控制器
- Connector：連接器
- Cable：纜線
...
## RFTools Utility（建議用法）
- Screen / Screen Module：螢幕 / 螢幕模組
- Energy / Power：能量
- Redstone：紅石
...
- Networked Pipe：網路管線
- Network：網路
- Route / Routing：路由 / 路由（建議固定用「路由」）
- Default Route：預設路由
- Destination：目的地
- Provider：供應（供應管線）
- Request：請求（請求管線 / 請求數量）
- Stocking：補給（補給管線 / 主動補給）
- Matching：匹配（匹配管線）
- Recipe：配方（配方管線 / 設定配方）
- Storage：儲存（儲存管線）
...
- Insert：輸入
- Eject：彈出 / 掉出（物品從管線掉出）
- Delete / Destroy：刪除 / 銷毀（物品已銷毀）
- Deploy：部署（像發射器一樣部署物品）
- Connect / Connected：連接 / 已連接
- Disconnect：中斷連接 / 斷開
- Adjust：調整（管線已調整）
- Request Amount：請求數量
- Cancel：取消
...
- Tag Filter：標籤過濾器
- Mod Filter：模組過濾器
- Match Components：匹配組件（耐久度、附魔等）
- Components：組件（耐久度、附魔等）
- Leave One：保留一個

### 模式與設定
- Active Stocking：主動補給
- Blocking Mode：一次一個（阻塞模式）
- Craftable：可合成
- Sort By：排序方式
- Ascending：低到高
- Descending：高到低
